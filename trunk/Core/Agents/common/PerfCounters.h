...
public:
	...
	T GetCounterValue(PERF_DATA_BLOCK **pPerfData, PERF_OBJECT_TYPE **pPerfObj, DWORD dwObjectIndex, DWORD dwCounterIndex, const LPWSTR pInstanceName = NULL)
	{
...
protected:
	...
	//
	T GetCounterValue(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, const LPWSTR pInstanceName)
	{
		...
		else
		{
			...
			wstring	strInstance;
			wstring	strInputInstance = pInstanceName;
			for( int k=0; k < pPerfObj->NumInstances; k++ )
			{
				strInstance = (wchar_t *)((PBYTE)pPerfInst + pPerfInst->NameOffset);
				if (strInstance == strInputInstance)
				{
